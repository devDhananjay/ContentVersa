...
import type { HuidQuotaStatus, HuidRecord } from "@/lib/goldverse/types";
...
export function HuidVerifyPanel({
  signInNext = "/goldverse#huid-verify",
}: {
  signInNext?: string;
}) {
  ...
  const loadQuota = React.useCallback(() => {
    fetch("/api/goldverse/huid/quota")
      .then((r) => r.json())
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setResult(null);
    try {
      const res = await fetch("/api/goldverse/huid/verify", {
        method: "POST",
